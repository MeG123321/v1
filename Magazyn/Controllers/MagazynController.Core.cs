...
public partial class MagazynController : Controller
{
    ...
    private List<TowarRodzajDto> GetRodzaje(System.Data.IDbConnection conn)
    {
        ...
        while (reader.Read())
            list.Add(new TowarRodzajDto { Id = Convert.ToInt64(reader["Id"]), Nazwa = reader["Nazwa"].ToString()! });
        return list;
    }
    ...
    private List<JednostkaMiaryDto> GetJednostkiMiary(System.Data.IDbConnection conn)
    {
        ...
        using var reader = cmd.ExecuteReader();
        while (reader.Read())
            list.Add(new JednostkaMiaryDto { Id = Convert.ToInt64(reader["Id"]), Nazwa = reader["Nazwa"].ToString()!, Skrot = reader["Skrot"] is DBNull ? null : reader["Skrot"].ToString() });
        return list;
    }
    ...
    private List<StawkaVatDto> GetStawkiVat(System.Data.IDbConnection conn)
    {
        ...
        using var reader = cmd.ExecuteReader();
        while (reader.Read())
            list.Add(new StawkaVatDto { Id = Convert.ToInt64(reader["Id"]), Nazwa = reader["Nazwa"].ToString()!, Wartosc = Convert.ToDouble(reader["Wartosc"]) });
        return list;
    }
}
